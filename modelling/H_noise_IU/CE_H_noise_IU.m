...
% using Glaze model with specified hazard rate (H), noise, and
% (in)consistency upweighting factor, and computes cross-entropy error term
% (e) with respect to observed responses.

% pm(1:3) = [H, noise, IU]

function e = CE_H_noise_IU(pm)
...
for p = 1:size(pm,1)
    ...
    CPs = Glaze_sim_fast_InconUp(LLRin,nsamps,pm(p,1),1,pm(p,2),pm(p,3));
