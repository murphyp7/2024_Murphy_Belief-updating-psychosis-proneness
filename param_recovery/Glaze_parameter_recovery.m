...
% given combination of H (hazard rate) and noise parameters, and
...
function [pm_fit,err] = Glaze_parameter_recovery(iter)
...
for t = 1:length(ntrials)
    for h = 1:length(Hs)
        for n = 1:length(noises)
            pm_in(end+1,:) = [ntrials(t) Hs(h) noises(n)];
        end
    end
end
...
for i = 1:nIters
    ...
    CPs = Glaze_sim_fast(LLRin,nsamps,H,1,noise);
    ...
    par_range = [range_p.H; range_p.noise];
    ...
    % Running PSO routine
    [output,~,~] = pso_Trelea_vectorized_Glaze('CE_H_noise',length(mv),mv,par_range,0,P,'goplotpso',PSOseedValue);
...
save([savepath,'Param_rec_H',num2str(H),'_noise',num2str(noise),'_t',num2str(nts),'.mat'],'pm_fit','err','muE','sigmaE','rangeE','nsamps','P')
